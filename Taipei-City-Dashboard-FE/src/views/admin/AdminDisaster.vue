...
<template>
  <div class="admindisaster">
    <!-- 1. Checkboxes to filter through different disaster types -->
    ...
    <table class="admindisaster-table">
      ...
      <tbody v-if="adminStore.disasters.length !== 0">
        ...
        >
          ...
          <td>{{ disaster.context ? disaster.context : "無" }}</td>
          <td>{{ disaster.status }}</td>
          <td>{{ parseTime(disaster.created_at) }}</td>
          <td>{{ disaster.updated_by }}</td>
          <td>{{ parseTime(disaster.updated_at) }}</td>
        </tr>
      </tbody>
      <!-- 2-2. Disaster are still loading -->
      <div
        v-else-if="contentStore.loading"
        class="disaster-nocontent"
      >
